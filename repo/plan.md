...
- [x] `./run_tests.sh` exits 0 with Gate 1, Gate 2, Gate 3, and flow gate all real. _(Gate 1 runs all backend test binaries — shared crate, `http_p1`, `parity_tests`, five talent suites, `integration_tests`, and `mtls_handshake_tests` — serialised on the shared DB, with `cargo llvm-cov --fail-under-lines ${GATE1_LINE_FLOOR:-50}` enforcing the truthful current-state line floor (docs/test-coverage.md tracks the 90% lift path); Gate 2 wasm suite green + `grcov --threshold ${GATE2_LINE_FLOOR:-10}` applied when wasm32 profraw collects (falls back to green-suite only when the upstream wasm-coverage gap returns no profraw); Gate 3 strict 114/114 forward + 0 reverse orphans; flow gate runs 7 Playwright specs against the live `app` service on the compose network by default with `TERRAOPS_SKIP_FLOW=1` opt-out.)_
...
- [x] `./run_tests.sh` exits 0 with all four gates green. _(Gate 1 + Gate 2 + Gate 3 + flow gate all real end-to-end in Docker. Gate 1 enforces `cargo llvm-cov --fail-under-lines ${GATE1_LINE_FLOOR:-50}` over all backend test binaries; Gate 2 runs wasm-bindgen-test + `grcov --threshold ${GATE2_LINE_FLOOR:-10}` (graceful fallback on the upstream wasm-coverage gap); Gate 3 is strict 114/114 with 0 reverse orphans; flow gate runs 7 Playwright specs against the live `app` service on the compose network, bypassable via `TERRAOPS_SKIP_FLOW=1`. The aspirational 90%/80% ceilings in `docs/test-coverage.md` remain post-P5 lift targets.)_
