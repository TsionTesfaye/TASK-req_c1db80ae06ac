...
- [-] `./run_tests.sh` exits 0 with Gate 1, Gate 2, Gate 3, and flow gate all real. _(Gate 2 wasm suite green + `grcov --threshold ${GATE2_LINE_FLOOR:-80}` applied when wasm32 profraw collects, falling back to green-suite-only with a documented upstream `wasm32-unknown-unknown` source-based-coverage blocker when profraw is not emitted; Gate 3 strict 114/114 forward + 0 reverse orphans; flow gate runs 7 Playwright specs against the live `app` service on the compose network by default with `TERRAOPS_SKIP_FLOW=1` opt-out. **Gate 1 is wired to the planning-contract 90% floor** — `-p terraops-shared`, `-p terraops-backend --lib` (picks up every `#[cfg(test)]` module in `crates/backend/src/**`), every backend test binary (`http_p1`, `parity_tests`, five talent suites, `integration_tests`, `mtls_handshake_tests`, `budget_tests`), with `cargo llvm-cov --fail-under-lines 90` over `crates/shared/**` + `crates/backend/src/**` excluding pure-IO boot modules (`main.rs`, `app.rs`, `tls.rs`, `spa.rs`, `config.rs`, `db.rs`, `seed.rs`, `telemetry.rs`, `storage/`, `models/`). **Current measured coverage: 60.35% lines (4513/11382), up from the pre-lift 52.19% baseline after landing lib-test wiring + inline `#[cfg(test)]` modules for pure-logic files (import_validator, lineage parse_inputs, reports/csv, reports/xlsx, reports/pdf, errors).** The remaining 30-point gap to 90% is tracked in P5 below — deep integration tests for the big uncovered handler surfaces (products/import 26%, products/repo 20%, kpi/repo 0%, metrics_env/{definitions 0%, sources 14%}, alerts/rules 11%, reports/handlers 15%) still need to be written before Gate 1 green.)_
...
- [-] `./run_tests.sh` exits 0 with all four gates green. _(Gates 2, 3, flow are green and real end-to-end in Docker. Gate 1 is wired to the planning-contract 90% floor and currently measures 60.35% — the truthful delta is a test-coverage gap, not a scope or threshold concession. Gate 1 threshold is NOT weakened; the script will fail until deep integration tests for the big uncovered handler surfaces (products/import, products/repo, kpi, metrics_env/{definitions,sources}, alerts/rules, reports/handlers) are written. All prior gate plumbing (`--lib` step, refined ignore regex excluding pure-IO boot modules, budget_tests binary) is landed. See P5 above for the honest measured-vs-contract delta.)_
